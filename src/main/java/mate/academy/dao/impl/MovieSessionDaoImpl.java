        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery(
                            "FROM MovieSession m WHERE m.movie.id = :movieId AND"
                                    + " m.showTime >= :start AND m.showTime < :nextStart",
                            MovieSession.class)
                    .setParameter("movieId", movieId)
                    .setParameter("start", date.atStartOfDay())
                    .setParameter("nextStart", date.plusDays(1).atStartOfDay())
                    .list();
        } catch (HibernateException e) {
            throw new RuntimeException(
                    "Can't find available session by movieID " + movieId + " and date " + date, e);
        }
    }
}
